  ...
  setfilter("experiance")
   setfiltervalue(filter)
    ...
    useEffect(() => {
        ...
        axios.post(`http://localhost:5000/users/freelancers?limit=${3}&offset=${Offset}`,{filter:filter , filtervalue:filtervalue})
            .then((resultinfo) => {
    ...
    const freelancersinfo =resultinfo.map((freelancer)=>{
       ...
       return (
        <div className='freelancerinfo' key={freelancer.id}>
         ...
         <div>
            <div>{freelancer.firstname}  {freelancer.lastname}     <span className='experince'> {freelancer.experiancename}</span> </div>
           
            <Stack spacing={1}>
            <Rating name="half-rating-read" defaultValue={freelancer.rate} precision={.5} readOnly />
            </Stack>
  ...
  return (
   ...
   >
    ...
    {state.majority.map((majority, i) => {
      return <option value={majority.id}>{majority.majorityname}</option>;
    })}
   </select>
...
{state.experiances.map((experiance, i) => {
 return <option value={experiance.id}>{experiance.experiancename}</option>;
})}
</select>
    ...
    <Stack spacing={2}>
      ...
      <Pagination count={Math.ceil(countfreelancers/3)} page={page} onChange={handleChange} />
    </Stack>
    </div>
    </div>
  )
